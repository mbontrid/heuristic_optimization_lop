...
void start_timers() {
  ...
  virtual_time = (double)res.ru_utime.tv_sec + (double)res.ru_stime.tv_sec +
...
void start_timers() {
  ...
  if (!GetProcessTimes(GetCurrentProcess(), &creation_time, &exit_time,
    ...
    exit(-1);
  virtual_time =
      user_time.dwLowDateTime / 1e7 + kernel_time.dwLowDateTime / 1e7;
...
TIMER_TYPE type;
{
  ...
  double vt = (double)user_time.dwLowDateTime / 1e7 +
              (double)kernel_time.dwLowDateTime / 1e7 - virtual_time;
  if (type == REAL) {
    DWORD x = GetTickCount();
